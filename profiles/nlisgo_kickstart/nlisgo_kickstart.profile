 ...
 */
function nlisgo_kickstart_image_default_styles() {
  $styles = array();
 ...
 */
function nlisgo_kickstart_form_install_configure_form_alter(&$form, $form_state) {
  ...
  $form['site_information']['site_name']['#default_value'] = st('nlisgo Site Kickstart');
  ...
  $form['admin_account']['account']['nlisgo_kickstart_name'] = array(
    '#type' => 'item',
    '#title' => st('Username'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['nlisgo_kickstart_password'] = array(
    '#type' => 'item',
    '#title' => st('Password'),
    '#markup' => 'admin'
  );
  $form['admin_account']['account']['nlisgo_kickstart_informations'] = array(
  ...
  array_unshift($form['#validate'], 'nlisgo_kickstart_custom_setting');
}
 ...
 * custom values.
 */
function nlisgo_kickstart_custom_setting(&$form, &$form_state) {
 ...
 */
function nlisgo_kickstart_system_info_alter(&$info, $file, $type) {
 ...
 */
function nlisgo_kickstart_update_projects_alter(&$projects) {
  ...
  $kickstart = clone $modules['nlisgo_kickstart'];
  $kickstart->info['hidden'] = FALSE;
  _update_process_info_list($projects, array('nlisgo_kickstart' => $kickstart), 'module', TRUE);
}
 ...
 */
function nlisgo_kickstart_update_status_alter(&$projects) {
  $bad_statuses = array(
    UPDATE_NOT_SECURE,
    UPDATE_REVOKED,
  ...
  $make_filepath = drupal_get_path('module', 'nlisgo_kickstart') . '/drupal-org.make';
  ...
  foreach ($projects as $project_name => $project_info) {
    ...
    if (strpos($project_name, 'nlisgo_kickstart_') !== FALSE) {
 ...
 *
 * Disable the update for nlisgo Site Kickstart.
 */
function nlisgo_kickstart_form_update_manager_update_form_alter(&$form, &$form_state, $form_id) {
  if (isset($form['projects']['#options']) && isset($form['projects']['#options']['nlisgo_kickstart'])) {
    if (count($form['projects']['#options']) > 1) {
      unset($form['projects']['#options']['nlisgo_kickstart']);
    }
    else {
      unset($form['projects']);
 ...
 */
function nlisgo_kickstart_crumbs_get_info() {
  $crumbs = array(
    'crumbs.home_title' => 0
  );

  foreach (module_implements('nlisgo_kickstart_crumb_info') as $module) {
    // The module-provided item might be just the name of the plugin, or it
    // might be an array in the form of $plugin_name => $weight.
    foreach (module_invoke($module, 'nlisgo_kickstart_crumb_info') as $crumb) {
      if (is_array($crumb)) {
        $crumbs += $crumb;
      }
      else {
